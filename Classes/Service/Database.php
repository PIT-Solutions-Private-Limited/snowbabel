...
class Database
{
     ...
     * @return null
     */
    public function getExtensions($CurrentTableId, $Conf = [])
    {
    ...
    public function getFiles($CurrentTableId, $Conf = false)
    {
        ...
        if (\is_array($Conf)) {
            ...
            // WHERE
            if ($Conf['ExtensionId']) {
                array_push($Where['AND'], $Table1.'.uid='.(int) ($Conf['ExtensionId']));
            }
        ...
        return $this->queryBuilder->getQueryBuilderForTable($Table1)
            ->select($Table1.'.ExtensionKey', $Table1.'.ExtensionTitle', $Table1.'.ExtensionDescription', $Table1.'.ExtensionCategory', $Table1.'.ExtensionIcon', $Table1.'.ExtensionLocation', $Table1.'.ExtensionPath', $Table1.'.ExtensionLoaded', $Table2.'.uid AS FileId', $Table2.'.ExtensionId', $Table2.'.FileKey')
            ->from($Table1)
            ->leftJoin(
               $Table1,
               $Table2,
     ...
     * @return null
     */
    public function getLabels($CurrentTableId, $Conf = [], $Count = false)
    {
        $Table1 = 'tx_snowbabel_indexing_extensions_'.$CurrentTableId;
        $Table2 = 'tx_snowbabel_indexing_files_'.$CurrentTableId;
     ...
     */
    public function getTranslations($CurrentTableId, $Conf = [], $Languages = [], $Count = false)
    {
        $Table1 = 'tx_snowbabel_indexing_extensions_'.$CurrentTableId;
        $Table2 = 'tx_snowbabel_indexing_files_'.$CurrentTableId;
        $Table3 = 'tx_snowbabel_indexing_labels_'.$CurrentTableId;
        ...
        if (\is_array($Conf) && \count($Conf) > 0) {
            ...
            if ($Conf['Sort']) {
                // Translations
                if (false !== strpos($Conf['Sort'], 'TranslationValue_')) {
        ...
        foreach ($Select as $val) {
            if (!\in_array($val['LabelName'], $key_array, true)) {
     ...
     * @return array|int|null|string
     */
    public function getTranslation($CurrentTableId, $Conf = [])
    {
        $Table1 = 'tx_snowbabel_indexing_extensions_'.$CurrentTableId;
        $Table2 = 'tx_snowbabel_indexing_files_'.$CurrentTableId;
        $Table3 = 'tx_snowbabel_indexing_labels_'.$CurrentTableId;
        ...
        if (\is_array($Conf) && \count($Conf) > 0) {
            ...
            if ($Conf['TranslationId']) {
                array_push($Where['AND'], $Table4.'.uid='.(int) ($Conf['TranslationId']));
            }
    ...
    private function insert($Table, $DataArray)
    {
        if (\is_array($DataArray) && \count($DataArray) > 0) {
            ...
            foreach ($DataArray as $Row) {
                $Row['tstamp'] = (string) (time());
                $Row['crdate'] = (string) (time());
    ...
    {
        if (\is_string($CommaSeparatedString) && false !== strpos($CommaSeparatedString, ',')) {
